...
def draw(comEmbed,traLabel,valLabel,name,path,i):
    ...
    axe1.set_title('tra', fontsize=90)
    
    axe2=plt.subplot(1,3,2)
    axe2.set_title('All', fontsize=90)
            
    axe3=plt.subplot(1,3,3)
    axe3.set_title('val', fontsize=90)
    ...
    plt.savefig(path+'Joint-sne_'+str(i)+'.pdf', dpi=300)
    plt.close()
    

    
def jointTsne(fea_train, class_train, fea_val, class_val, name=[''],path='./',alpha=8):
    ...
    '''
    # how many batches => how many pics
    for i in range(1, len(paint_fea_train)):
        traFvce = paint_fea_train[i]
        valFvec = paint_fea_train[i]

        comFvec = np.concatenate((traFvce, valFvec), axis=0)
